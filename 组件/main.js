...
var addSql_componentMenuNav = "insert into ued_component_menu_nav(menu_id,name,label,level,parent) values(?,?,?,?,?)";
var truncate ="truncate table ued_component_menu_nav";
var datas = transToArrayFromObj(componentFiles);

insertData(datas);

function insertData(datas){
    dbConnection.query(truncate,function (err, result) {
        if(err){
